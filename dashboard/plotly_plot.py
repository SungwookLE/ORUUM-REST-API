...
def dash_plotly_plot(mysql_connection, update_date):
    """
    ...
    """
    #Create SQL command string
    sql = """SELECT * FROM api_stock_list WHERE update_date = '{}'""".format(update_date)
    ...
    #Create graph object Figure object with data
    fig = go.Figure(data = go.Scatter(name = 'ORUUM' + update_date, x = dash_plot_table['ticker'], y = dash_plot_table['prevclose']))
...
mysql_connection = pymysql.connect(host="127.0.0.1",user='root',password='3102',db='oruum_db',cursorclass=pymysql.cursors.DictCursor)

#Get plot options by running SQL query
update_date_options_sql = "SELECT update_date FROM api_stock_list"
...
app.layout = html.Div([
                html.Div([
                    ...
                    dcc.Dropdown(
                      id = 'update_date',
                      options = [{'label': i, 'value': i} for i in update_date_options],
...
def display_value(update_date):
    ...
    fig = dash_plotly_plot(mysql_connection, update_date)
    return fig