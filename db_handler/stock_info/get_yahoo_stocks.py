...
class YahooStockCrawler(GetTicker, YahooStockFunctionSet):
    ...
    def get_stocks_information_history(self, yearly=True, quarterly=True):
        ...
        progress_bar = tqdm(tickers_df)
        progress_bar.set_description(f"{self.market}, stockinformation-history")
        ...
        while (tickers_df.empty is not True):
            ...
            dataframe_yearly_balance_sheet_data = pd.concat([(dataframe_Raw_financial_data["yearly_balance_sheet"])], axis=0)
            dataframe_yearly_cash_flow_data = pd.concat([(dataframe_Raw_financial_data["yearly_cash_flow"])], axis=0)
            ...
            # Quarterly Data
            dataframe_quarterly_income_statement_data = pd.concat([(dataframe_Raw_financial_data["quarterly_income_statement"])], axis=0)
            dataframe_quarterly_balance_sheet_data = pd.concat([(dataframe_Raw_financial_data["quarterly_balance_sheet"])], axis=0)
            dataframe_quarterly_cash_flow_data = pd.concat([(dataframe_Raw_financial_data["quarterly_cash_flow"])], axis=0)
